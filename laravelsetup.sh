...
if [ ! -f "/vagrant/composer.json" ]; then
  ...
  git clone https://github.com/laravel/laravel.git tmp && mv tmp/{.,} ./ && rm -rf tmp
  rm readme.md 
  ...
  # Install and Configure way/generators, itsgoingd/clockwork, fzaninotto/faker, phpunit/phpunit packages
  echo -e "\n"
...
cd /vagrant
mv gruntwatchlesslivereload/{.,}* public/ && rm -rf gruntwatchlesslivereload 
cd public/
